...
if [ -d "${HOME}/cmake-install" ]; then
	...
	if [ "${TRAVIS_OS_NAME:-}" == "linux" ] || uname -s | grep -i Linux &> /dev/null; then
		...
		wget --no-check-certificate -qO - https://cmake.org/files/v3.3/cmake-3.3.2-Darwin-x86_64.tar.gz | tar -xz
		# copy to a directory that will not changed every version
		mv cmake-3.3.2-Darwin-x86_64/CMake.app/Contents ${HOME}/cmake-install
	else
